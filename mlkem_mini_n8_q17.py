...
def ByteEncode(F: list[int], d: int) -> list[bytes]:
    ...
    for i in range(n):
        ...
        for j in range(d):
            b[i * d + j] = a & 1
            a >>= 1
    return BytesBits.BitsToBytes(b)



def ByteDecode(B: bytes, d: int, k: int) -> list[int]:
    #バイト列からビット列に直し、dビットの塊を2進数の数字として解釈
    #入力：圧縮されたバイト列
    #出力：k個のリスト(要素はn次元多項式)
    b = BytesBits.BytesToBits(B)
    polys = []
    offset = 0
...
def k_pke_keygen(seed: bytes) -> tuple:
    ...
    #1.乱数生成[p,S]
    p,S = HashFunctions.hash_G(seed, k)
...
def message_equal(m: bytes, M:bytes, tolerance: int = 7):
    #復号メッセージ　M　が元メッセージ　m　と誤差　tolerance内で一致するか確認
    for mb, Mb in zip(m, M):
        if abs(mb - Mb) > tolerance:
            return False
    return True
...
def main():
    ...
    print("[*] 本来のメッセージ:",m.hex())
    print("[*] 秘密鍵によるメッセージ:", M.hex())

    if message_equal(m, M):
        print("復号成功: 誤差範囲内で一致")
    else:
        print("復号失敗: 誤差が大きい")
